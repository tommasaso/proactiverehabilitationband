...
void loop() {
  ...
  //while loop per determinare se l'esercizio è iniziato entro 30s dall'accensione della fascia
  while (current_time <= start_time+period && flag1 == true)  {
    current_time = millis();
    CatchAngles(); 
    if (max_angle >= th) {
      Serial.print("L'esercizio è iniziato ");Serial.print("\n");
      flag1 = false;
      analogWrite( motor , 0 );    // spegne la vibrazione 
      Serial.print("spengo vibrazione");Serial.print("\n");
    }  
  }
    
  if (flag1 == true){    //non ha raggiunto la soglia. Parte timer per la vibrazione. uscito da while perché non è iniziato esercizio
    for(int times=0; times<=2; times++) {//vibra per tre volte con un ritardo di 60s tra una vibrazione e l'altra a meno che non venga raggiunta la soglia. Bisogna controllare i delay!
       Serial.print("non ha ancora iniziato esercizio");Serial.print("\n");
       Serial.print("spengo vibrazione");Serial.print("\n");
       analogWrite( motor , 0 );    // spegne la vibrazione
       delay(20000); // nella realtà di dieci minuti
       Serial.print("accendo vibrazione");Serial.print("\n");
       period = 20000;
       ...
       while (current_time <= wait_time+period && flag1 == true)  {
          Serial.print("vibra per un minuto");Serial.print("\n");
          current_time = millis();
          CatchAngles();
          if (max_angle >= th) {
            Serial.print("L'esercizio è iniziato ");Serial.print("\n");
...
//    FunctionsPitchRoll(Ax, Ay, Az);   //Calcolo angolo Roll e Pitch
//    current_angle = Roll;
//    Serial.print("SPERIAMOBENE");Serial.print("\n");
//    if (current_angle>goal){
//      Serial.print("Raggiunto l'obiettivo, mantenere la posizione");Serial.print("\n");
//      startbuzzer();
//      //while (current_time <= start_time+period){ //per 10 secondi devo mantenere la posizione
//        //salvare i dati mandandoli al server
//      //}
//      startbuzzer();
//      Serial.print("esercizio finito");Serial.print("\n");
//      delay(60000); //tempo che serve per ricominciare il tutto
//      flag1 = true;
//      }
//      
//    }
      ...
      start_time = millis();
      current_angle = Roll;
      period = 30000; // da caricare dal server
